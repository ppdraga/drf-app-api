...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_full_recipe_update(self):
        ...
        # tag was not provided in update
        tags = recipe.tags.all()
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_bad_request(self):
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, {'image': 'dummy'}, format='multipart')
        self.assertEqual(res.status_code, status.HTTP_400_BAD_REQUEST)

    def test_filter_recipes_by_tags(self):
        recipe1 = sample_recipe(user=self.user, title='Recipe_1')
        recipe2 = sample_recipe(user=self.user, title='Recipe_2')
        recipe3 = sample_recipe(user=self.user, title='Recipe_2')
        tag1 = sample_tag(user=self.user, name='Tag_1')
        tag2 = sample_tag(user=self.user, name='Tag_2')
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        recipe2 = sample_recipe(user=self.user, title='Recipe_2')
        recipe3 = sample_recipe(user=self.user, title='Recipe_2')
        ingredient1 = sample_ingredient(user=self.user, name='Ingredient_1')
        ingredient2 = sample_ingredient(user=self.user, name='Ingredient_2')
